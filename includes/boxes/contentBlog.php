...
?>
	...
	if ( !get_post_format() || has_post_format( 'quote' , get_the_id()) || has_post_format( 'link' , get_the_id())) {?>
		...
		<?php if(marketeer_getImage(get_the_id(), 'marketeer-lite-postGridBlock-2') != '' && (!isset($postmeta["marketeer_featured_category"][0]) || $postmeta["marketeer_featured_category"][0] == 1)) { ?>	
			<div class="blogimage">		
				<?php 
				...
				<?php if(has_post_format( 'link' , get_the_id())) { ?>	
					<a href="<?php echo esc_url($link); ?>" rel="bookmark" title="Permanent Link to <?php the_title_attribute(); ?>"><?php echo marketeer_getImage(get_the_id(), 'marketeer-lite-postGridBlock-2'); ?></a>
				<?php } if (has_post_format( 'quote' , get_the_id())) { ?>
					<?php  echo marketeer_getImage(get_the_id(), 'marketeer-lite-postGridBlock-2'); ?>
				<?php }  if (!get_post_format() ) { ?>
					<a href="<?php the_permalink() ?>" rel="bookmark" title="Permanent Link to <?php the_title_attribute(); ?>"><?php echo marketeer_getImage(get_the_id(), 'marketeer-lite-postGridBlock-2'); ?></a>
				<?php } ?>
			</div>
		<?php } ?>
